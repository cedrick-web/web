...
export default function App() {
  ...
  const freeById=useMemo(()=>new Map(freeChallenges.map(c=>[`DSP-${String(c.id).padStart(3,'0')}`,c])),[]);
  const done=completed.length;
  const percent=Math.round(done/60*100);
  const go=v=>{setView(v);setModule(null);setTool(null)};
  const openModule=m=>{setModule(m);setTool(null);setView('module')};
  const openTool=t=>{setTool(t);setModule(null);setView('tool')};
  const toggle=id=>{const next=completed.includes(id)?completed.filter(x=>x!==id):[...completed,id];setCompleted(next);saveCompletedChallenges(next)};
  const moduleDone=m=>{const first=Number(m.range.match(/DSP-(\d+)/)?.[1]||1);return Array.from({length:m.count},(_,i)=>`DSP-${String(first+i).padStart(3,'0')}`).filter(id=>completed.includes(id)).length};
  return <div className="app-shell"><aside className="sidebar"><button className="brand brand-button" onClick={()=>go('dashboard')}><span className="brand-mark">D</span><span>DevSprint</span></button><div className="side-label">LEARNING SPACE</div><nav className="side-nav">{nav.map(([id,label])=><button key={id} className={view===id||(id==='modules'&&view==='module')||(id==='tools'&&view==='tool')?'active':''} onClick={()=>go(id)}><span>{id==='dashboard'?'⌂':id==='modules'?'▦':id==='challenges'?'◇':id==='tools'?'⚙':'◔'}</span>{label}</button>)}</nav><div className="sidebar-progress"><span>Your progress</span><strong>{done}/60</strong><div className="progress-track"><div className="progress-fill" style={{width:`${percent}%`}}/></div><small>{percent}% complete</small></div><div className="sidebar-note">V1 · JavaScript Problem-Solving &amp; DSA Toolkit</div></aside><main className="learning-main"><header className="topbar"><div><span className="mobile-brand">DEVSPRINT</span><span className="breadcrumb">Learning space / {view==='module'?module?.title:view==='tool'?tool?.title:view==='progress'?'Progress':view==='tools'?'Labs & Tools':view==='challenges'?'Challenges':'Dashboard'}</span></div><div className="top-progress"><span>{done} completed</span><div className="mini-progress"><div style={{width:`${percent}%`}}/></div></div></header>{view==='dashboard'&&<Dashboard modules={modules} tools={tools} done={done} percent={percent} onModule={openModule} onTool={openTool} onView={go}/>} {view==='modules'&&<Modules modules={modules} moduleDone={moduleDone} onModule={openModule}/>} {view==='module'&&module&&<ModuleDetail module={module} completed={completed} moduleDone={moduleDone(module)} titles={challengeTitles} freeById={freeById} open={open} setOpen={setOpen} back={()=>go('modules')} challenges={()=>go('challenges')}/>} {view==='challenges'&&<Challenges titles={challengeTitles} freeById={freeById} completed={completed} open={open} setOpen={setOpen} toggle={toggle}/>} {view==='tools'&&<Tools tools={tools} onTool={openTool}/>} {view==='tool'&&tool&&<ToolDetail tool={tool} back={()=>go('tools')} project={()=>openModule(modules[7])}/>} {view==='progress'&&<Progress modules={modules} moduleDone={moduleDone} done={done} percent={percent} onModule={openModule}/>}</main></div>;
}
...
function ToolCard({tool,onClick}){return <button className="tool-card clickable" onClick={onClick}><span className="tool-icon">{tool.icon}</span><div><span className="tool-status">{tool.status}</span><h3>{tool.title}</h3><p>{tool.description}</p></div><span className="card-arrow">→</span></button>}
